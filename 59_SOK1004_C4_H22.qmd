...
Grunnen til dette kan være at da blir ikke alle verdiene slått sammen, men det blir mellomrom mellom verdiene, og dermed blir det tydelig å lese av histogrammet.

Eksperimenter med forskjellige verdier for `binwidth` og forklar hva som kjennetegner en god verdi.

Det som kjennetegner en god verdi er når det blir tydelig å kunne lese av figuren. Når søylene ikke blir for tykke, men heller ikke for tynne. Ved å gjøre test kan det se ut som at binwidth=0.1 er den beste verdien, ettersom da ser vi alle søylene hver for seg, og ingen av søylene blir slått sammen.
...
df1 <- df %>%
  filter(sysselsettingsgruppe == c("200-499 sysselsatte", "500 sysselsatte og over" )) %>%
  filter(statistikkvariabel == "FoU-kostnader i alt")

df1$år <- as.integer(df1$år)
