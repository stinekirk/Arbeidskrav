...
# her gjør jeg slik at jeg kun får opp liste 1 i df
df <- df[[2]]

#Her endrer jeg tid fra chr til en int
df <- df %>%
  mutate(Tid=parse_number(Tid))

#Her endrer jeg navn på variablene
df <- df %>%
  rename(var= ContentsCode, tid = Tid, verdi = value)

#Her gjør jeg om df til en tibble
df <- as_tibble(df)
...
#Her lager jeg en figur, og bruker filter for å vis kun tidsperioden fra 2000 til 2021. I tillegg filtrer jeg slik at jeg får løpende priser, ved å skrive var == "BNP"

df %>%
   filter(tid %in% tid) %>% 
  filter(tid >= 2000:2021) %>%
  filter(var == "BNP") %>%

#Legger til navn på y og x-akse, i tillegg velger jeg hvilken farge som skal være på grafen

  ggplot(aes(x=tid, y=verdi)) +
  
  geom_line(color = "dark red") +

#Legger til navn på hele figuren
  labs(title= "BNP i løpende priser mellom 2000 og 2021", x= "tid", y="BNP i 1000 kr") +
  theme_bw()


#Gjør det samme om igjen, men her filtrer jeg slik at det kun brukes memobnp, for å kunne få BNP i faste priser.
...
df %>%
  ...
  labs(title = "BNP i faste priser mellom 2000 og 2021", x="tid", y="BNP i 1000 kr") +
  theme_bw()

# Når man måler BNP i løpende priser vil man ta utgangspunkt i de faktiske prisene det gjeldene året. Når det gjøres målinger ved faste priser så tar det høyde for inflasjon og endret pengeverdi, og da ser man kun på produksjonsutviklingen. https://www.ageras.no/ordbok/bruttonasjonalprodukt

#Ved å se på de to grafene kan vi se at inntektene har hatt en mer jevn stigning ved løpende priser enn ved faste priser.Ved de faste prisene har det vært større svigninger, enn i de løpende prisene. 
...
# Ved å sammenligne bnp og bnp_wide kan vi se at alle data typene endres til "int" ved bnp_wide. Man ser ikke lenger variabelen sine navn, men byttes til type. Gjor den lange tabellen kortere, ved bruk av wide. 
```
...
# Viser den prosentvise endringen i BNP,og deler opp i nominativ og relativ vekst. 

df_wide <- df_wide %>%
  mutate(DBNP = 100*(BNP-LBNP)/LBNP) %>%
  mutate(DMEMOBNP= 100 *(BNP-LMEMOBNP)/LMEMOBNP)
```
...
#Her endrer jeg navn på variabelen til df_long, og bruker funksjonen privot_longer, til å få tilbake de gamle varianelnavnene "var" og "verdi"
...
# Bruker filter funksjonen for å velge nominell vekst. Ved å begrense tiden velger jeg også å bruke filter funksjonen. Videre bruker jeg ggplot, til å legg inn y og x-akase. og dermed bruker jeg geom_point og legger inn hvilken farge jeg vil ha på grafen. og geom_smooth for å får prikkene i grafen. 

df_long %>%
   filter(var== "DBNP") %>% 
  filter(tid %in% tid) %>%
  filter(tid >= 1970:2021) %>%
  ...
  geom_line(color = "dark blue") +
 labs(title= "Prosentvis vekst i nominelt BNP per person fra 1970 til 2021", x= "tid", y="Endring i prosent") +
  theme_bw()
...
df_long %>%
  ...
  labs(title= "Prosentvis vekst i reelt BNP per person fra 1970 til 2021", x="tid", y="Endring i prosent")+
  theme_bw()
